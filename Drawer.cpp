...
void Drawer::DrawTwoSignals(vector<double>& signal1, vector<double>& signal2, double fd)
{
	...
	CPen subgrid_pen(PS_DOT, 1, RGB(200, 200, 200));
	CPen grid_pen(PS_SOLID, 1, RGB(0, 0, 0));
	CPen data_pen1(PS_SOLID, 4, RGB(200, 0, 0));
...
void Drawer::DrawTwoGraph(vector<double>& signal1, vector<double>& signal2, double fd, int shift, double cap)
{
	if (!init) return;
	...
	CPen data_pen1(PS_SOLID, 3, RGB(200, 0, 0));
	CPen data_pen2(PS_SOLID, 2, RGB(0, 0, 200));
	CPen cap_pen(PS_SOLID, 2, RGB(0, 200, 0));
	...
	memDC.SelectObject(&data_pen2);
	signal2 = convert_range(signal2, actual_top, actual_bottom, data_y_max, data_y_min);
	memDC.MoveTo(convert_range(shift/fd, actual_right, actual_left, data_x_max, data_x_min), signal2[0]);
	for (int i = 0; i < signal2.size(); i++)
	{
		double x = convert_range((i + shift) / fd, actual_right, actual_left, data_x_max, data_x_min);
		memDC.LineTo(x, signal2[i]);
	}


	memDC.SelectObject(&cap_pen);
	memDC.MoveTo(convert_range(data_x_min, actual_right, actual_left, data_x_max, data_x_min), convert_range(cap, actual_top, actual_bottom, data_y_max, data_y_min));
	memDC.LineTo(convert_range(data_x_max, actual_right, actual_left, data_x_max, data_x_min), convert_range(cap, actual_top, actual_bottom, data_y_max, data_y_min));
